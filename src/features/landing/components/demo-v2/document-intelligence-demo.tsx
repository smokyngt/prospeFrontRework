...
import { Safari } from '@/components/workspace-ui/shared';
import { Button } from '@/components/workspace-ui/ui/button';
import { Empty } from '@/components/workspace-ui/ui/empty';
import { TooltipProvider } from '@/components/workspace-ui/ui/tooltip';
import {
  applyLandingTheme,
...
function DocumentPanel({ onClose }: { onClose: () => void }) {
  ...
  const file = demoFileById(activeTab?.fileId);
...
export function IntelligenceDemo({
   ...
   * visite guidée sans clic supplémentaire (choix de secteur sur l'accueil).
   */
  autoOpenTick?: number;
  /** Appelé quand la visite guidée se termine ou est passée. */
  onTourEnd?: () => void;
  /** Remplace le contenu du voile d'ouverture affiché au survol de l'aperçu. */
  overlay?: ReactNode;
  ...
  const handleTour = useCallback(
    ...
    [isRunning, onTourEnd, runEnterEffect],
  );
   ...
   * que le jeu de données du secteur soit monté avant de lancer la visite.
   * Le tick traité est mémorisé pour qu'un simple changement de langue — qui
   * recrée `openInteractiveDemo` — ne relance pas la démo.
   */
  const handledOpenTickRef = useRef(0);

  useEffect(() => {
    if (!autoOpenTick || !mounted || handledOpenTickRef.current === autoOpenTick) {
      return;
    }
    handledOpenTickRef.current = autoOpenTick;
    const timeout = window.setTimeout(() => openInteractiveDemoRef.current(), 80);
    return () => window.clearTimeout(timeout);
  }, [autoOpenTick, mounted]);
                ...
                >
                  {showThreadPanel ? (
                    <ThreadSidebar
                      ...
                      archivedCount={chatConfig.archivedThreadCount}
