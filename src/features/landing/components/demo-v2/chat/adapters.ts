...
import type { Citation, Hallucination, OrchestrationStep, Store } from '@/lib/workspace-sdk-types';
...
export function toOrchestrationStep(
  step: DemoStep,
  index: number,
  delays: number[],
): OrchestrationStep {
  const startedAt = delays
    .slice(0, index)
    .reduce((sum: number, delay: number) => sum + delay, 0);
  return {
    action: step.action,
    chunks: step.chunks,
