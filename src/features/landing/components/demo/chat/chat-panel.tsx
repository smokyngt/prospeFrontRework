...
import { Markdown, content as markdownContent } from '@/components/shared/markdown';
...
import { ChatInput, EFFORT_LEVELS } from './input';
...
import type { MarkerKind } from '@/components/shared/markdown-markers';
...
export function ChatMessages({
  ...
  const { i18n, t } = useTranslation('chat');
  const ui = useMemo(() => demoUi.get(i18n.language), [i18n.language]);
  const [effort, setEffort] = useState<number>(EFFORT_LEVELS[2]);
  ...
  const orchestrationSteps = useMemo(
    () => steps.map((step, index) => toOrchestrationStep(step, index)),
    [steps],
  );
          ...
          <Sheet open={filterOpen} onOpenChange={setFilterOpen}>
            <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-md">
              <SheetHeader className="border-b border-border/70 px-4 py-3">
                <SheetTitle className="text-sm">{t('filter.panel_title')}</SheetTitle>
              </SheetHeader>
              <Tabs defaultValue="stores" className="flex flex-1 flex-col overflow-hidden">
                ...
                >
                  <div className="space-y-0.5 px-2 pb-2">
                    <button
                      className={cn(
                        'flex w-full cursor-pointer items-center gap-1.5 rounded-md px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent',
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate">{ui.papers}</span>
                    ...
                    {demoChatConfig.filterFiles.map((file) => (
                      <button
                        key={file}
                        className="flex w-full cursor-pointer items-center gap-1.5 rounded-md px-2 py-1.5 pl-10 text-left text-sm transition-colors hover:bg-accent"
                        type="button"
                      >
                    ...
                    {demoChatConfig.filterFolders.map((folder) => (
                      <button
                        key={folder.label}
                        className="flex w-full cursor-not-allowed items-center gap-1.5 rounded-md px-2 py-1.5 text-left text-sm opacity-45"
                        disabled
                        type="button"
                      >
                ...
                >
                  <div className="space-y-2 p-3">
                    {demoChatConfig.filterMetadata.length > 0 ? (
                      demoChatConfig.filterMetadata.map(
                        (entry: { key: string; value: string }, index: number) => (
                          <div key={`${entry.key}-${index}`} className="flex items-center gap-1.5">
                            <Input
                              value={entry.key}
                              readOnly
                              placeholder={t('metadata_filter.field', 'Metadata field')}
                              className="h-8 min-w-0 flex-1 rounded-none border-r-0 text-xs"
                            />
                            <Select defaultValue="eq">
                              <SelectTrigger className="h-8 w-28 rounded-none border-r-0 text-xs">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            <Input
                              value={entry.value}
                              readOnly
                              placeholder={t('metadata_filter.value', 'Value')}
                      ...
                      <p className="py-2 text-center text-xs text-muted-foreground">
                        {t('metadata_filter.empty', 'No metadata rules added.')}
                      </p>
                    )}
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t('metadata_filter.add', 'Add rule')}
                    </Button>
                  </div>
                </TabsContent>
              </Tabs>
          ...
          <ChatInput
            ...
            renderFilterSlot={() => (
              <Button
                ...
                size="sm"
                className={cn(
                  'h-8 gap-1.5 px-2.5 text-xs hover:bg-primary/5',
                  selectedFilterCount > 0 && 'text-primary',
                )}
                onClick={() => setFilterOpen((open) => !open)}
              >
                <Filter className="h-3.5 w-3.5" />
                <span className="hidden text-xs sm:inline">{t('filter.label')}</span>
                {selectedFilterCount > 0 && (
